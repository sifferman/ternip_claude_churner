...
**What:** build_2 surfaced a 7-LUT-level self-loop on
...
**Why:** Surfaced as cluster B in build_2 (`tmatmul_operation_q[1]`
→ `latched_tmatmul_addrs_q.CE`). Was masked by build_1's wide-CE
cluster. Even with build_2 reverted, this is the second-tier path.
...
"things to try" list. We never hit it in build_0..build_3 because
the `rms → convert` cluster dominated first, but with build_1's
...
**Why:** OneCore at WNS=-0.259 (build_1) isn't quite there yet, but
...
extend (e.g., the build_1 input slice could have analogues on
